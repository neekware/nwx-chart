...
async function buildPackage() {
  if (program.build) {
    ...
    await execute(cmd).catch(error => {
      console.log(`Failed to build @nwx/chart ... ${error}`);
      return false;
    });
  }
  return true;
...
program
  ...
  .option('-b, --build', 'Build @nwx/chart')
  .option('-p, --publish', 'Publish @nwx/chart@latest')
  .option('-d, --dev', 'Publish @nwx/chart@dev')
  .parse(process.argv);
